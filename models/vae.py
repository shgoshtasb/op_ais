...
from .decoder import Base, WuFCDecoder1, WuFCDecoder2, WuFCDecoder3, BinaryDecoder, ConvDecoder, DCGANGenerator
from .encoder import WuFCEncoder1, WuFCEncoder2, WuFCEncoder3, BinaryEncoder, ConvEncoder, DCGANEncoder
...
class VAE(Base):
    ...
    def get_encoder(self):
        if self.net == 'wu-wide':
            self.encoder_net = WuFCEncoder1(self.data_shape, 2 * self.latent_dim)
        elif self.net == 'wu-small':
            self.encoder_net = WuFCEncoder2(self.data_shape, 2 * self.latent_dim)
        elif self.net == 'wu-shallow':
            self.encoder_net = WuFCEncoder3(self.data_shape, 2 * self.latent_dim)
        elif self.net == 'binary':
            self.encoder_net = BinaryEncoder(self.data_shape, 2 * self.latent_dim)
        elif self.net == 'conv':
            self.encoder_net = ConvEncoder(nn.GELU, 2 * self.latent_dim, 
                                            self.data_shape[0], self.data_shape[1])
        elif self.net == 'dcgan':
            self.encoder_net = DCGANEncoder(self.data_shape[1], 
                                            self.data_shape[0], latent_dim=2 * self.latent_dim)
        else:
            raise NotImplemented
        if self.device > 0:
            self.encoder_net.cuda()
        if self.device > 1:
            self.encoder_net = nn.DataParallel(self.encoder_net, range(self.device))

                                   
    def get_decoder(self):
        output_shape = self.data_shape * np.array([self.dc, 1, 1])
        if self.net == 'wu-wide':
            self.decoder_net = WuFCDecoder1(output_shape, self.latent_dim)
        elif self.net == 'wu-small':
            self.decoder_net = WuFCDecoder2(output_shape, self.latent_dim)
        elif self.net == 'wu-shallow':
            self.decoder_net = WuFCDecoder3(output_shape, self.latent_dim)
        elif self.net == 'binary':
            self.decoder_net = BinaryDecoder(output_shape, self.latent_dim)
        elif self.net == 'conv':
            self.decoder_net = ConvDecoder(nn.GELU, self.latent_dim, 
                                           output_shape[0], output_shape[1])
        elif self.net == 'dcgan':
            self.decoder_net = DCGANGenerator(output_shape[1], output_shape[0],
                                              self.latent_dim)
        else:
            raise NotImplemented
        if self.device > 0:
        ...
        if self.device > 1:
            self.decoder_net = nn.DataParallel(self.decoder_net, range(self.device))
