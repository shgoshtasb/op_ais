...
class Flow(Bijective):
    ...
    def sample(self, n_samples, x=None):
        ...
        eps = base_dist.sample(torch.Size([n_samples,]))
        ...
        z, log_J = self(eps, x)
        return z, eps_log_prob - log_J
    
    def log_prob(self, z, x=None):
        base_dist = torch.distributions.Normal(loc=self.zero, scale=self.one)
        eps, log_J = self(z, x, mode='inverse')
        eps_log_prob = base_dist.log_prob(eps).sum(dim=-1, keepdim=True)
...
class Permute(Bijective):
    ...
    def forward_(self, x, context, log_J):
        return torch.index_select(x, 1, self.permutation), log_J
    
    def backward_(self, x, context, log_J):
        return torch.index_select(x, 1, self.inverse_permutation), log_J
...
class AffineCoupling(Bijective):
    ...
    def forward_(self, x, context, log_J):
        ...
        log_J = log_J + logs.sum(dim=-1, keepdim=True)
        return x, log_J
...
class RealNVP(Flow):
    ...
    def forward_(self, x, context, log_J):
        for i, module in enumerate(self.transforms):
            x, log_J = module(x, context, log_J, mode='direct')
        return x, log_J
    
    def backward_(self, x, context, log_J):
        for i, module in enumerate(reversed(self.transforms)):
            x, log_J = module(x, context, log_J, mode='inverse')
        return x, log_J
...
class Gaussian(Flow):
    ...
    @property
    def scale(self):
        return torch.exp(.5 * self.logvar)
    
    @property
    def entropy(self):
        return 0.5 * (self.input_dim * np.log(2 * np.pi) + self.input_dim + self.logvar.sum(dim=-1, keepdim=True))