...
from sampling.transitions import MH, HMC, MALA, ULA
...
def get_schedule(M, schedule):
    if schedule == 'geometric':
        if M == 1:
            ...
            return torch.tensor(np.geomspace(.001, 1., M))
    elif schedule == 'linear':
        return torch.tensor(np.linspace(0., 1., M+1)[1:])
    elif schedule == 'sigmoid':
        scale = 0.3
        s = torch.sigmoid(torch.tensor( \
                    np.linspace(-1./scale, 1./scale, M + 1)))
