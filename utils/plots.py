...
def plot_particles(ax, z, log_w=None, legend=False, lims=LIMS, nb_point_per_dimension=100, cmap="coolwarm"):
    if log_w is not None:
        ...
        normalized_w = normalized_w / torch.sum(normalized_w, dim=0)
        scaled_w = torch.exp(log_w - torch.max(log_w, dim=0)[0]).cpu()
        ...
        print(torch.min(scaled_w), torch.max(scaled_w), torch.mean(scaled_w))
        colors = cm.rainbow(np.linspace(0, 1, 101))
        heatmap = ax.scatter(z[:,0].cpu(), -z[:,1].cpu(), c=scaled_w, cmap=cmap, s=marker__sizes)#, rasterized=True)
    else:
